...
$ecoEstrellas          = $ecoEstrellas          ?? [];
$ecoTareasLimpieza     = $ecoTareasLimpieza     ?? [];
...
<div class="eco-wrap">
    ...
    <!-- ── Estrellas del equipo (al final, con equivalente en soles) ── -->
    <p class="eco-sec-title">⭐ Estrellas del equipo — <?= $mesLabel ?></p>
    <?php
    $fmtEst = fn($v) => (floor($v) == $v) ? (string)(int)$v : number_format($v, 1);
    $ecoEstrellasFiltro = $ecoPid
        ? array_values(array_filter($ecoEstrellas, fn($e) => (int)$e['id'] === $ecoPid))
        : $ecoEstrellas;
    ?>
    <?php if (empty($ecoEstrellasFiltro)): ?>
    ...
    <div class="eco-table-wrap">
        <table class="eco-table">
            ...
            <tbody>
            <?php foreach ($ecoEstrellasFiltro as $e):
    ...
    <!-- ── Administración: actividades de limpieza y valor en estrellas ── -->
    ...
    <div class="eco-table-wrap">
        <table class="eco-table">
            ...
            <tbody id="tlTbody">
            <?php foreach ($ecoTareasLimpieza as $t): ?>
    ...
    <div style="background:#fff;border:1.5px solid #fbcfe8;border-radius:10px;padding:.9rem 1rem;margin-top:.75rem;">
        <p style="font-size:.75rem;font-weight:600;color:#475569;margin-bottom:.5rem;">Agregar nueva actividad</p>
        <div style="display:flex;gap:.5rem;flex-wrap:wrap;align-items:flex-end;">
            <div style="min-width:120px;flex:1;">
                <label style="font-size:.65rem;font-weight:700;text-transform:uppercase;letter-spacing:.05em;color:#64748b;display:block;margin-bottom:2px;">Código</label>
                <input type="text" id="tlNuevoCodigo" placeholder="VITRINAS"
                       style="padding:.4rem .65rem;border:1.5px solid #fbcfe8;border-radius:7px;font-size:.8rem;width:100%;box-sizing:border-box;text-transform:uppercase;">
            </div>
            <div style="min-width:180px;flex:2;">
                <label style="font-size:.65rem;font-weight:700;text-transform:uppercase;letter-spacing:.05em;color:#64748b;display:block;margin-bottom:2px;">Descripción</label>
                <input type="text" id="tlNuevoDesc" placeholder="Limpiar vitrinas"
                       style="padding:.4rem .65rem;border:1.5px solid #fbcfe8;border-radius:7px;font-size:.8rem;width:100%;box-sizing:border-box;">
            </div>
            <div style="min-width:100px;">
                <label style="font-size:.65rem;font-weight:700;text-transform:uppercase;letter-spacing:.05em;color:#64748b;display:block;margin-bottom:2px;">Estrellas máx.</label>
        ...
        <div id="tlMsg" style="font-size:.78rem;padding:.4rem .75rem;border-radius:7px;margin-top:.6rem;display:none;"></div>
    </div>
...
const BASE_TL = window.location.pathname.split('/admin/')[0];
const tlApiUrl = (p) => `${window.location.origin}${BASE_TL}${p}`;

function tlShowMsg(msg, ok) {
...
async function tlGuardar(id) {
    ...
    const r   = await fetch(tlApiUrl(`/admin/api/tarea-limpieza/${id}/actualizar`), {
        method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(data),
    });
    const res = await r.json();
    if (res.success) { tlShowMsg('Actividad actualizada.', true); setTimeout(() => location.reload(), 900); }
    else tlShowMsg(res.message || 'Error.', false);
}

async function tlToggle(id) {
    const r   = await fetch(tlApiUrl(`/admin/api/tarea-limpieza/${id}/toggle`), { method: 'POST' });
    const res = await r.json();
...
async function tlAgregar() {
    ...
    if (!data.codigo || !data.descripcion || !data.estrellas_max) {
        tlShowMsg('Completa código, descripción y estrellas máximas.', false);
        return;
    }
    const r   = await fetch(tlApiUrl('/admin/api/tarea-limpieza/agregar'), {
        method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(data),
    });
    const res = await r.json();
    if (res.success) { tlShowMsg('Actividad agregada.', true); setTimeout(() => location.reload(), 900); }
    else tlShowMsg(res.message || 'Error.', false);
}
</script>
